...
蝉圈圈组件库参考资料放在 `references/`。只有当用户、项目、仓库或 Figma 文件明确和蝉圈圈设计系统相关时才加载它；如果用户没提到蝉圈圈，且无法从项目、仓库或 Figma 上下文判断当前是什么项目，要先问用户目前做的是什么项目，不要自行套用蝉圈圈规范。需要查规范时，先看 `references/index.md`；默认优先读取 `references/chanquanquan-cheatsheet.md`，只有需要完整色阶、组件细节、来源说明或兜底推导时，才读取 `references/chanquanquan-design.md`。
...
1. 先读精确设计节点：
   ...
   - 已映射项目组件可以作为权威实现，但必须确认其渲染结果与 Figma 中的 icon、图片、状态和视觉属性一致；如果不一致，要回到 Figma 原始资产或精确节点属性。
...
3. 判断并读取设计规范：
   ...
   - 只有蝉圈圈相关任务，或用户明确要求使用蝉圈圈组件库时，才读取 `references/index.md` 并按索引路由到 `references/chanquanquan-cheatsheet.md` 或 `references/chanquanquan-design.md`。
   - 如果用户没提到蝉圈圈，且无法从项目、仓库或 Figma 上下文判断当前是什么项目，要先问用户目前做的是什么项目，不要自行套用蝉圈圈规范。
...
4. 保留真实 Figma 资产：
   ...
   - 交付前要建立简短资产清单，记录原始资产、资产类型、落地路径和使用位置；如果某个资产缺失，要记录缺失资产和当前处理方式。
...
6. 实现前端布局和交互：
   ...
   - 如果有匹配的 Code Connect 组件，并且适合目标节点，优先使用它；但不得因为复用组件而静默改变 Figma icon、图片、mask 或状态资产。
...
7. 浏览器视觉验收：
   ...
   - 每次视觉验收至少记录目标 viewport，并选取 3-5 个视觉锚点进行对比，例如关键按钮位置、标题字号、卡片边界、主色、icon 是否存在、表格行高或主要图片裁切。
...
先读取项目本地设计规则，例如 `DESIGN.md`、设计 token、theme 文件、组件文档或仓库指令。只有当任务属于蝉圈圈组件库，或用户明确要求使用蝉圈圈规范时，才加载蝉圈圈参考文档。如果用户没提到蝉圈圈，且无法从项目、仓库或 Figma 上下文判断当前是什么项目，要先问用户目前做的是什么项目。

蝉圈圈相关任务先读取 `references/index.md`。默认先查 `references/chanquanquan-cheatsheet.md` 的高频值；需要完整说明、完整色阶、组件细节、来源说明或补充推导时，再按需读取 `references/chanquanquan-design.md`：

- 快速默认值：`chanquanquan-cheatsheet.md`。
- 品牌气质和整体风格：速查表章节 `1`；完整规范章节 `1` 和 `7`。
- 颜色、token 匹配、状态色：速查表章节 `2`、`3`；完整规范章节 `2`、`6.1`、`6.2`。
- 字体：速查表章节 `4`；完整规范章节 `3` 和 `6.4`。
- 按钮、卡片、数据卡、导航、表格、表单、标签、图表、Banner：速查表章节 `5`、`6`、`8`；完整规范对应 `4.x` 章节。
- 页面布局和缺失 token 的兜底规则：速查表章节 `6`、`9`；完整规范章节 `5`、`8`。
- 渐变：速查表章节 `8`；完整规范章节 `2.5` 和 `6.3`。
...
- 资产清单已记录原始资产、资产类型、落地路径、使用位置和缺失项处理。
